import { d as registerInstance, e as h } from './counting-othello-65d3ba3b.js';
    ...
    (function () {
        ...
        var runInContext = (function runInContext(context) {
            ...
            function baseSortedIndexBy(array, value, iteratee, retHighest) {
                ...
                var low = 0, high = array == null ? 0 : array.length, valIsNaN = value !== value, valIsNull = value === null, valIsSymbol = isSymbol(value), valIsUndefined = value === undefined$1;
                while (low < high) {
...
function generateField(x, y) {
    var xArray = new Array(x).fill(0);
    var yArray = new Array(y).fill(0);
    var field = yArray.map(function () { return lodash.clone(xArray); });
    field[x / 2][y / 2] = 1;
    field[x / 2 - 1][y / 2 - 1] = 1;
    field[x / 2 - 1][y / 2] = 2;
    field[x / 2][y / 2 - 1] = 2;
...
function calcReverseField(x_position, y_position, field, maxVal) {
    if (maxVal === void 0) { maxVal = 1000; }
    var pointVal = field[y_position][x_position];
    var newField = lodash.cloneDeep(field);
    var upArr = _getArr(x_position, y_position, newField, 0);
    var dArr = _getArr(x_position, y_position, newField, 4);
    var rArr = _getArr(x_position, y_position, newField, 2);
    var lArr = _getArr(x_position, y_position, newField, 6);
    var urArr = _getArr(x_position, y_position, newField, 1);
    var drArr = _getArr(x_position, y_position, newField, 3);
    var dlArr = _getArr(x_position, y_position, newField, 5);
    var ulArr = _getArr(x_position, y_position, newField, 7);
    console.log(dlArr);
    var nupArr = calcReverse(pointVal, upArr, maxVal);
    var ndArr = calcReverse(pointVal, dArr, maxVal);
    var nrArr = calcReverse(pointVal, rArr, maxVal);
    var nlArr = calcReverse(pointVal, lArr, maxVal);
    var nurArr = calcReverse(pointVal, urArr, maxVal);
    var ndrArr = calcReverse(pointVal, drArr, maxVal);
    var ndlArr = calcReverse(pointVal, dlArr, maxVal);
    var nulArr = calcReverse(pointVal, ulArr, maxVal);
    nupArr.concat(ndArr).concat(nrArr).concat(nlArr)
        .concat(nurArr).concat(ndrArr).concat(ndlArr).concat(nulArr).forEach(function (point) { return newField[point.y][point.x] = point.pointVal; });
    return newField;
}
function _getArr(x, y, field, dir) {
    ...
    while (_moveAndGetPoint(_x, _y, field, dir).isMove) {
        var p = _moveAndGetPoint(_x, _y, field, dir);
        arr.push(p);
        _x = p.x;
        _y = p.y;
    }
 ...
 * @param arr
 */
function calcReverse(pointVal, arr, maxVal) {
    if (maxVal === void 0) { maxVal = 1000; }
    var newArr = lodash.cloneDeep(arr);
...
var Othello = /** @class */ (function () {
    function Othello(hostRef) {
     ...
     * コンポーネントロード時発火する関数
     */
    Othello.prototype.componentWillLoad = function () {
        this.field = generateField(this.x_length, this.y_length);
    };
    Othello.prototype.xWatch = function () {
        this.field = generateField(this.x_length, this.y_length);
    };
    Othello.prototype.yWatch = function () {
        this.field = generateField(this.x_length, this.y_length);
    };
    /**
     * オセロをおいたときの処理
     ...
     * @param y
     */
    Othello.prototype.clickSlot = function (e, x, y) {
        e.stopPropagation();
        if (this.field[y][x] !== 0) {
            return;
        }
        var newField = lodash.cloneDeep(this.field);
        newField[y][x] = this.field[y][x] + this.player + 1;
        newField = calcReverseField(x, y, newField, this.maxval);
        if (lodash.flatten(this.field).reduce(function (a, b) { return a + b; }) + this.player + 1
            == lodash.flatten(newField).reduce(function (a, b) { return a + b; })) {
            return;
        }
        this.field = newField;
   ...
   * @param event
   */
    Othello.prototype.handleMaxValChange = function (event) {
    ...
    Othello.prototype.render = function () {
        ...
        return h("div", null, h("table", null, this.field.map(function (xArr, yIndex) { return h("tr", null, xArr.map(function (num, xIndex) { return h("th", { class: [
            ...
            ? 'レッドの番だよ!!'
            : 'ブルーの番だよ！！'), h("div", null, 'レッドのセルの数：' + this.countCellNum('red')), h("div", null, 'ブルーのセルの数：' + this.countCellNum('blue')), h("div", null, 'レッドのセルの中の合計値：' + this.countCellSum('red')), h("div", null, 'ブルーのセルの中の合計値：' + this.countCellSum('blue')), h("div", { class: "cp_iptxt" }, "\u8A31\u3055\u308C\u308B\u30BB\u30EB\u306E\u5024\u306E\u6700\u5927\u5024\uFF1A", h("input", { type: "number", value: this.maxval, onChange: function (event) { _this.handleMaxValChange(event); } })), h("button", { class: "btn-square-little-rich", onClick: function () { return _this.reset(); } }, "\u30EA\u30BB\u30C3\u30C8"));
    };
    Object.defineProperty(Othello, "watchers", {
        get: function () {
            return {
                "x": ["xWatch"],
                "y": ["yWatch"]
