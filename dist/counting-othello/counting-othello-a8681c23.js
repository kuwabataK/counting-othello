...
const loadModule = (cmpMeta, hostRef, hmrVersionId) => {
    ...
    return __sc_import_counting_othello(
...
const patchEsm = () => {
    ...
    if (!(win.CSS && win.CSS.supports && win.CSS.supports('color', 'var(--c)'))) {
        // @ts-ignore
        return __sc_import_counting_othello('./css-shim-229ebf7a-229ebf7a.js');
    }
...
const patchBrowser = async () => {
    ...
    if (importMeta !== '') {
        ...
        if (!window.customElements) {
            // @ts-ignore
            await __sc_import_counting_othello('./dom-7eb7509a-7eb7509a.js');
        }
...
const proxyComponent = (Cstr, cmpMeta, isElementConstructor, proxyState) => {
    if (BUILD.member && cmpMeta.$members$) {
        ...
        members.forEach(([memberName, [memberFlags]]) => {
            if ((BUILD.prop && (memberFlags & 31 /* Prop */)) || (BUILD.state && proxyState && (memberFlags & 32 /* State */))) {
                // proxyComponent - prop
            ...
            else if (BUILD.lazyLoad && BUILD.method && isElementConstructor && (memberFlags & 64 /* Method */)) {
    ...
    if ((BUILD.lazyLoad || BUILD.style) && (hostRef.$flags$ & 256 /* hasInitializedComponent */) === 0) {
        ...
        if (BUILD.style && !Cstr.isStyleRegistered && Cstr.style) {
            ...
            if (!BUILD.hydrateServerSide && BUILD.shadowDom && cmpMeta.$flags$ & 8 /* needsShadowDomShim */) {
                style = await __sc_import_counting_othello('./shadow-css-984bac74-549b16dd.js').then(m => m.scopeCss(style, scopeId, false));
            }
