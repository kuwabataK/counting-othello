...
function calcReverse(x_position, y_position, field) {
    const newField = lodash.cloneDeep(field);
    const upArray = [...Array(y_position).keys()].map(i => newField[x_position][i]).reverse();
    const downArray = [...Array(newField.length - y_position - 1).keys()]
        .map(i => newField[x_position][y_position + 1 + i]);
}
